...
int registro ()
{
	...
	scanf("%s",cpf);// %s lê cada caracter e armazena na string
...
int deletar ()
{
    ...
    file= fopen(cpf, "r");
    fclose(file);
    if(file == NULL)//Verifica se o CPF existe no banco de dados, se não exibe mensagem de erro.
    {
    	printf(" O usuário não se encontra cadastrado em nosso sistema!\n");
    	system("pause");
	}
